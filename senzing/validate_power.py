...
Validates the Senzing Kiro Power structure for the lightweight MCP
...
class PowerValidator:
    ...
    def validate_file_structure(self) -> bool:
        ...
        # Minimal steering structure - one guide on using the MCP server
    ...
    def validate_metadata(self) -> bool:
        ...
        # Valid Power-Builder fields (as of 1.1.0)
        valid_fields = [
            "name",
            "displayName",
            "description",
        ...
        # Check for invalid fields (not in valid_fields list)
        for line in frontmatter.split('\n'):
            if ':' in line:
                field = line.split(':')[0].strip()
                if field and field not in valid_fields:
                    msg = (
                        f"Invalid metadata field "
                        f"(not in Power-Builder spec): {field}"
                    )
                    self.log("WARNING", msg)

        # Validate keywords (should be array)
        keywords_match = re.search(
            r'^keywords:\s*\[(.*?)\]', frontmatter, re.MULTILINE
        )
        if keywords_match:
            ...
            if len(keywords) > 10:
                msg = (
                    ...
                    f"Recommend 5-7 specific terms."
                )
                self.log("WARNING", msg)
    ...
    def validate_internal_links(self) -> bool:
        ...
        if all_valid:
            ...
            msg = f"Found {len(broken_links)} broken internal links"
            self.log("ERROR", msg)
    ...
    def validate_mcp_config(self) -> bool:
        ...
        if "mcpServers" not in config:
            ...
            if "senzing-mcp-server" not in servers:
                msg = "mcp.json missing 'senzing-mcp-server' configuration"
                self.log("ERROR", msg)
                all_valid = False
            else:
                ...
                if "url" not in server_config:
                    msg = "senzing-mcp-server missing 'url' field"
                    self.log("ERROR", msg)
                    all_valid = False
                else:
                    url = server_config["url"]
                    if not url.startswith("https://"):
                        msg = f"MCP server URL should use HTTPS: {url}"
                        self.log("WARNING", msg)
                ...
                if "timeout" in server_config:
                    ...
                    if timeout < 30000:
                        msg = (
                            f"Timeout ({timeout}ms) may be too short "
                            f"for MCP operations"
                        )
                        self.log("WARNING", msg)
    ...
    def validate_power_philosophy(self) -> bool:
        """Validate lightweight activation layer philosophy"""
        msg = "Validating power philosophy (lightweight design)..."
        self.log("INFO", msg)
        ...
        if lines > 200:
            msg = (
                f"POWER.md is {lines} lines. Consider if content "
                f"should be in MCP server instead."
            )
            self.log("WARNING", msg)
        else:
    ...
    def run_all_validations(self) -> bool:
        ...
        print("\n" + "="*60)
        print("Senzing Power Validation (v1.1 - Lightweight Design)")
        print("="*60 + "\n")
...
def main():
    parser = argparse.ArgumentParser(
        description="Validate Senzing Kiro Power (v1.1 - Lightweight Activation Layer)"
    )
